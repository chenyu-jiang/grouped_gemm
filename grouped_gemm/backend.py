...
def get_ptrs(a, b, c, batch_sizes, trans_b):
    a_ptrs = torch.empty(batch_sizes.shape[0], dtype=torch.uint64)
    b_ptrs = torch.empty(batch_sizes.shape[0], dtype=torch.uint64)
    c_ptrs = torch.empty(batch_sizes.shape[0], dtype=torch.uint64)
